%load data
load('Segmented_Machining_ToolWear.mat');
...
for i = 1:420
    ...
    for j = 1:6
        ...
        fn=fieldnames(ToolWearCrop);
        
        
        %values needed for crest factor, shape factor and impulse factor
        %max value
        mx = max(ToolWearCrop(i).(fn{j}));
        %RMS
        root_ms = rms(ToolWearCrop(i).(fn{j}));
        %absolute mean value
        abs_mean = meanabs(ToolWearCrop(i).(fn{j}));
        ...
        %mean
        result(i).(name{1}) = mean(ToolWearCrop(i).(fn{j}));
        %standard deviation
        result(i).(name{2}) = std(ToolWearCrop(i).(fn{j}));
        %root mean square
        result(i).(name{3}) = root_ms;
        %kurtosis
        result(i).(name{4}) = kurtosis(ToolWearCrop(i).(fn{j}));
        %skewness
        result(i).(name{5}) = skewness(ToolWearCrop(i).(fn{j}));
        %peak-to-peak
        result(i).(name{6}) = peak2peak(ToolWearCrop(i).(fn{j}));
        %crest factor
        ...
        %energy
        result(i).(name{11}) = meansqr(ToolWearCrop(i).(fn{j}));
...
filename = 'extracted_features_toolwear.xlsx';
